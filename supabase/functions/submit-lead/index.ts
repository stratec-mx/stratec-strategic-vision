...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (countErr) {
      console.error("[submit-lead] rate_limits count error:", countErr.message);
      // Fail open — do not block the user for a DB error
    } else if ((count ?? 0) >= RATE_LIMIT_MAX) {
      return new Response(
        JSON.stringify({
          error: "Ha alcanzado el límite de solicitudes. Por favor espere 15 minutos antes de intentarlo de nuevo.",
        }),
        {
          status: 429,
          headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
